...
from app.crud.book_crud import delete_by_id, insert, select_all, select_by_id
...
def test_insert_success(setup_clear_db):
    ...
    with session.begin():
        ...
        ret: Book = insert(
            session,
            BookCreateParam(title="Integration Test Book", author_id=expect_author.id),
        )
...
def test_select_all_success(setup_clear_db):
    ...
    with session.begin():
        ...
        ret1: Book = insert(
            session,
            BookCreateParam(
                title="Integration Test Book1", author_id=expect_authors[0].id
            ),
        )
        ret2: Book = insert(
            session,
            BookCreateParam(
                title="Integration Test Book2", author_id=expect_authors[1].id
            ),
        )
        ret3: Book = insert(
            session,
            BookCreateParam(
                title="Integration Test Book3", author_id=expect_authors[1].id
            ),
        )
