...
def dummy_create_author_service(session: Session, param: AuthorCreateParam):
    return Author(id=uuid4(), name=param.name)


def test_create_author_success(override_dependency):
    app.dependency_overrides[get_create_author_service] = provider(
        dummy_create_author_service
    )
...
def test_create_author_fail_length(override_dependency):
    ...
    response = client.post("/authors", json={"name": "1234567890" * 5 + "1"})
    data = response.json()
    assert response.status_code == 422
    assert f"{data['detail'][0]['ctx']}" == "{'max_length': 50}"
